...
We can plot both the MSFT and S&P500 price movement. If we recall our analysis at 
...
Just by looking at the plot it seems that the MSFT and S&P500 are `correlated`,
...
we'll do a regression.

We can see that on average MSFT (`0.175%`) has a higher daily return compared to
the S&P500 (`0.099%`).

As we could previously see from just from the plot, that it seemed that there was
...
For the p-value we got the value `2e-16`s so we could reject the null hypothesis
with a high confidence. 

We the `scatter plot` of the returns and the `regression line` once again we
can clearly see the high level of `positive association` between MSFT and S&P500
returns.
...
A we already knew from previous steps, returns are highly correlated and there
