...
#### Best fit model - 3M and Nasdaq
```{r, echo=FALSE}
# Winning model data transformation
model_winner = MSFT_daily_ret_original~MMM_retDaily+Nasdaq_retDaily
df_winner = as.data.frame(cbind(MSFT_daily_ret_original, MMM_retDaily, Nasdaq_retDaily))
```
...
#### Runner up model - Nasdaq and S&P 500
```{r, echo=FALSE}
# Runner-up model data transformation
model_runner_up = MSFT_daily_ret_original~SP500_retDaily+Nasdaq_retDaily
df_runner_up = as.data.frame(cbind(MSFT_daily_ret_original, SP500_retDaily, Nasdaq_retDaily))
```
...
#### Third model - Google and Nasdaq
```{r, echo=FALSE}
# Third model data transformation
model_third = MSFT_daily_ret_original~GOOG_retDaily+Nasdaq_retDaily
df_third = as.data.frame(cbind(MSFT_daily_ret_original, GOOG_retDaily, Nasdaq_retDaily))
```
...
```{r, echo=FALSE}
plot(as.numeric(MSFT_daily_ret_test), type="l",col="black", main="One-period-ahead forecasts", xlab="Days ahead", ylab="Returns")
lines(pred_vals_1_winner, type="l", pch=22, lty=2, col="red")
#lines(pred.vals.1.runner_up, type="l", pch=22, lty=2, col="green")
#lines(pred.vals.1.third, type="l", pch=22, lty=2, col="blue")

#legend( 
#  "topright", inset=c(-0.06, 0), text.col=c("black", "red","green", "blue"), 
#  legend=c("Test sample","3M-Nasdaq","S&P500-Nasdaq","GOOGLE-Nasdaq")
#)
```

Based on the results above, we can see that we now have the reverse order of best matching models.

The RMSE (Root Mean Squared Error) is lowest for the third regression model.

It is still tight race, but for the one-period-ahead forecast, the winner is the third model which has `GOOGLE` and `Nasdaq` as explanatory variables in regression model.
...
Let's compare new favorite model with two others:

`Comparison GOOGLE-Nasdaq and SP500-Nasdaq`
```{r echo=FALSE}
dm.test(errors_1_third, errors_1_runner_up, "less", h=1)
```

`Comparison GOOGLE-Nasdaq and 3M-Nasdaq`
```{r echo=FALSE}
dm.test(errors_1_third, errors_1_winner, "less", h=1)
```

Based on the results above, and the p-values (below 5%), we can state that we have a winner for one-period-ahead forecast and that is the third model from previous section (`GOOGLE-Nasdaq` regression model).
...
```{r, echo=FALSE}
plot(as.numeric(MSFT_daily_ret_test), type="l",col="black", main="Multi-period-ahead forecasts", xlab="Days ahead", ylab="Returns")
lines(pred_vals_5_winner, type="l", pch=22, lty=2, col="red")
#lines(pred.vals.5.runner_up, type="l", pch=22, lty=2, col="green")
#lines(pred.vals.5.third, type="l", pch=22, lty=2, col="blue")
...
Based on the results above, we can see that we now have the reverse order of best matching models again.
...
`Comparison GOOGLE-Nasdaq and SP500-Nasdaq`
```{r echo=FALSE}
dm.test(errors_5_third, errors_5_runner_up, "less", h=5)
```

`Comparison GOOGLE-Nasdaq and 3M-Nasdaq`
```{r echo=FALSE}
dm.test(errors_5_third, errors_5_winner, "less", h=5)
```
...
After evaluating forecast performance in the previous section, and after we performed the tests, we can conclude that regression mode that has `GOOGLE` and `Nasdaq` as explanatory is the best fit.

Here is the brief overview of the key metrics for that model: 

|    Metric                 |       Google - Nasdaq         |
|---------------------------|:-----------------------------:|
| **AIC**                   |         -3401.212             |
| **BIC**                   |         -3384.313             |
| **RMSE (1-period-ahead)** |         0.009104144           |
| **RMSE (5-period-ahead)** |         0.009110567           |
...
Regression model `Google-Nasdaq` is better for each of the metric parameter.

Therefore, **the Ultimate winner is regression model GOOGLE-Nasdaq!**