...
Looking at the Standardised Resituals Tests, we conclude the following:
  * The Jarque-Bera Test p-value is zero (5.497-08) so we don't have normal distribution.
  * There no evidence of correlation in our residuals and there's no dependence in conditional variance.
  * The p-value for the LM Arch Test is 0.72 (not rejecting the null) which means that there's no additional ARCH effect our model didn't captured.
``` {r warning=FALSE, echo=FALSE}
...
Looking at the Standardised Resituals Tests, we conclude the following:
  ...
  * There no evidence of correlation in our residuals and there's no dependence in conditional variance.
  * The p-value for the LM Arch Test is 0.68 (not rejecting the null) which means that there's no additional ARCH effect our model didn't captured.
```{r echo=FALSE, warning=FALSE}
GARCH_Model_skewed_student_t = garchFit(~arma(1, 1) + garch(2, 1), data = training_set, trace=F, cond.dist = 'sstd')

summary(GARCH_Model_skewed_student_t)
...
# GARCH_Model_GeneralizedErrorDist
```
...
``` {r echo=FALSE, warning=FALSE}
IGARCH_Model <- ugarchspec(mean.model = list(armaOrder=c(1, 1)),
                           variance.model = list(model = 'iGARCH', garchOrder = c(2, 1)),
                           distribution = 'std')

IGARCH_Model <- ugarchfit(spec = IGARCH_Model, data= na.omit(training_set))
...
We first do a forecast using the selected model, we use rolling forecast method and predict entire out-of-sample set.
``` {r echo=FALSE, warning=FALSE}
fit_roll <- ugarchfit(GARCH_Model_skewed_student_t_spec, 
                      data = na.omit(original_set),
                      out.sample = 110)

forecast <- ugarchforecast(fit_roll, 
                            n.ahead = 1, 
                            n.roll = 110)
...
The Root-mean-square error is 0.018215. 
``` {r echo=FALSE, warning=FALSE}
