...
#### Fit the Gaussian ARMA–GARCH model
...
#### Fit ARMA(0, 0) - eGARCH(1, 1) model with Student t-distribution 
``` {r}
MSFT_garch_1 <- ugarchspec(mean.model = list(armaOrder=c(0,0)),
...
#### Fit ARMA(2, 2) - eGARCH(1, 1) model with Student t-distribution
``` {r}
MSFT_garch_3 <- ugarchspec(mean.model = list(armaOrder=c(2,2)),
                           variance.model = list(model = 'eGARCH',garchOrder = c(1, 1)),
                           distribution = 'std')
...
TODO
``` {r}
Model = c('fit_garch_1','fit_garch_2','fit_garch_3')
AIC = c(-5.4199, -5.4448, -5.4387)
...
``` {r}
fit_roll <- ugarchfit(MSFT_garch_2, 
