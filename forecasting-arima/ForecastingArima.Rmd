...
For evaluation model forecast performance, we will use `MSFE` 
...
arma23_forecast <- matrix(0L, nrow = H, ncol=2)

# Rolling scheme
j <- 0
for (j in 0:(H-1)) {
  arma23 <- arima(original_set[(1+j):(T+j)], order = c(2, 0, 3))
  forctemp <- predict(arma23, 1)
  arma23_forecast[j+1,1] <- forctemp$pred
}
...
arma22_forecast <- matrix(0L, nrow = H, ncol=2)

# Rolling scheme
j <- 0
for (j in 0:(H-1)) {
  arma22 <- arima(original_set[(1+j):(T+j)], order = c(2, 0, 2))
  forctemp <- predict(arma22, 1)
  arma22_forecast[j+1,1] <- forctemp$pred
}
...
arma21_forecast <- matrix(0L, nrow = H, ncol=2)

# Rolling scheme
j <- 0
for (j in 0:(H-1)) {
  arma21 <- arima(original_set[(1+j):(T+j)], order = c(2, 0, 1))
  forctemp <- predict(arma21, 1)
  arma21_forecast[j+1,1] <- forctemp$pred
}
...
ar8_forecast <- matrix(0L, nrow = H, ncol=2)

# Rolling scheme
j <- 0
for (j in 0:(H-1)) {
  ar8 <- arima(original_set[(1+j):(T+j)], order = c(8, 0, 0))
  forctemp <- predict(ar8, 1)
  ar8_forecast[j+1,1] <- forctemp$pred
}
